...
CREATE TABLE status (
    ...
    UNIQUE(name, status_type_id) -- Added UNIQUE constraint
);
...
-- Tables linking User to Farm (farm_id stored, but no FK constraint to farm table)
CREATE TABLE user_role_farm (
    user_role_farm_id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    role_id integer NOT NULL,
    user_id integer NOT NULL,
    farm_id integer NOT NULL, -- Belongs to Farm microservice domain
    status_id integer NOT NULL,
...
CREATE TABLE notifications (
    ...
    date timestamp NOT NULL,
    user_id integer NOT NULL,
    invitation_id integer, -- FK within this service's scope
    notification_type_id integer, -- FK within this service's scope
    farm_id integer NOT NULL, -- Belongs to Farm microservice domain
    status_id integer NOT NULL -- FK within this service's scope
);
...
ALTER TABLE notifications
    ADD CONSTRAINT fk_notifications_invitation_id FOREIGN KEY (invitation_id) REFERENCES invitation(invitation_id) ON DELETE SET NULL; -- Or ON DELETE CASCADE depending on desired behavior

-- Note: Foreign key constraints for farm_id in user_role_farm, invitation, and notifications
-- are intentionally omitted as 'farm' table belongs to a different microservice.
